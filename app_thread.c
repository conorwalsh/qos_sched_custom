...
app_mixed_thread(struct thread_conf **confs)
{
	...
	while ((conf = confs[conf_idx])) {
		...
		static uint64_t en_count = 0;
		static uint64_t de_count = 0;
		if (likely(nb_pkt)) {
			#ifdef QOSSTATS
		...
		uint64_t delta_count = en_count - de_count;

		static uint64_t QOSDESPIN = 0;

		//if(rte_sched_run()>0||delta_count>50){	
		if(rte_get_tsc_cycles()>=QOSDESPIN||delta_count>30){

			nb_pkt = rte_sched_port_dequeue(conf->sched_port, mbufs,
					                burst_conf.qos_dequeue);
			de_count += (uint64_t)nb_pkt;

			uint64_t USECDELAY = 0;
			uint64_t CYCLESPERSEC = rte_get_timer_hz();
			uint64_t CYCLESPERUSEC = CYCLESPERSEC/1000000;
			uint64_t CYCDELAY = CYCLESPERUSEC*USECDELAY;
			QOSDESPIN=rte_get_tsc_cycles()+CYCDELAY;		
		}
		else{
			nb_pkt = 0;
		}

		CYCLE_PUT_DELTA(delta_count);
